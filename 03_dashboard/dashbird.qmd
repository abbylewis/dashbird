...
output$plot_comp <- renderPlotly({
  comb_plot <- all_personal %>%
    ...
    facet_wrap(~year, scales = "free_x")+
    ylab("Number of species")+
    scale_color_manual(values = c("#4ECDC4","#29363D","#E54B4B", "purple"))+
    scale_x_date(date_labels = "%b", 
                 expand = c(0,0))+
    theme(panel.grid.major = element_line(color = "grey95"),
          axis.title.x = element_blank(),
...
output$abby <- renderUI({ 
  Abby = unique(all_personal$`Common Name`[all_personal$Observer=="Abby Lewis"])
  Other = unique(all_personal$`Common Name`[!all_personal$Observer=="Abby Lewis"])
  Abby_text <- HTML(c("<b>Abby's unique birds</b><br>",paste(Abby[!Abby%in%Other], collapse = "<br>")))
  Abby_text
})
...
output$sue <- renderUI({ 
  ...
  Sue_text <- HTML(c("<b>Sue's unique birds</b><br>",paste(Sue[!Sue%in%Other], collapse = "<br>")))
  Sue_text
})

output$bjorn <- renderUI({ 
  Bjorn = unique(all_personal$`Common Name`[all_personal$Observer=="Bjorn Larson"])
  Other = unique(all_personal$`Common Name`[!all_personal$Observer=="Bjorn Larson"])
  Bjorn_text <- HTML(c("<b>Bjorn's unique birds</b><br>",paste(Bjorn[!Bjorn%in%Other], collapse = "<br>")))
  Bjorn_text
})

output$eric <- renderUI({ 
  Eric = unique(all_personal$`Common Name`[all_personal$Observer=="Eric Larson"])
  Other = unique(all_personal$`Common Name`[!all_personal$Observer=="Eric Larson"])
  Eric_text <- HTML(c("<b>Eric's unique birds</b><br>",paste(Eric[!Eric%in%Other], collapse = "<br>")))
  Eric_text
})
