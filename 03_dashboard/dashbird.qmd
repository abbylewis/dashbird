...
theme_dashbird <- egg::theme_article()+
...
h4("Plot Specifications")
radioButtons(inputId = "venn_years",
             label = "To plot:",
             choices = c("All",
                         "2026",
                         "2025",
...
h4("Plot Specifications")
radioButtons(inputId = "map_years",
             label = "To plot:",
             choices = c("All",
                         "2026",
                         "2025",
...
everything <- read_csv("https://raw.githubusercontent.com/abbylewis/dashbird/refs/heads/main/02_extracted_data/for_dashbird.csv", show_col_types = F)
...
to_map <- everything %>%
  ...
  distinct()%>%
  mutate(year = year(Date))%>%
  group_by(Location, Latitude, Longitude, Observer, year)%>%
  summarize(n = n())
...
output$venn <- renderPlot({
  years <- if(input$venn_years == "All"){2024:year(Sys.Date())} else {as.numeric(input$venn_years)}
  venn_data <- all_personal %>%
    filter(year %in% years)
  ggVennDiagram::ggVennDiagram(x = list(
    Abby = unique(venn_data$`Common Name`[venn_data$Observer=="Abby Lewis"]),
    Bjorn = unique(venn_data$`Common Name`[venn_data$Observer=="Bjorn Larson"]),
    Eric = unique(venn_data$`Common Name`[venn_data$Observer=="Eric Larson"]),
    Sue = unique(venn_data$`Common Name`[venn_data$Observer=="Susan Lewis"])
  ),
  label_geom = "text",
  label_color="white",
  label_alpha = 0.9,
...
output$unique_birds <- DT::renderDT({ 
  ...
  Abby = unique(venn_data$`Common Name`[venn_data$Observer=="Abby Lewis"])
  Other = unique(venn_data$`Common Name`[!venn_data$Observer=="Abby Lewis"])
  Abby_text <- HTML(paste(Abby[!Abby%in%Other], collapse = "<br>"))
  
  Sue = unique(venn_data$`Common Name`[venn_data$Observer=="Susan Lewis"])
  Other = unique(venn_data$`Common Name`[!venn_data$Observer=="Susan Lewis"])
  Sue_text <- HTML(paste(Sue[!Sue%in%Other], collapse = "<br>"))
  
  Bjorn = unique(venn_data$`Common Name`[venn_data$Observer=="Bjorn Larson"])
  Other = unique(venn_data$`Common Name`[!venn_data$Observer=="Bjorn Larson"])
  Bjorn_text <- HTML(paste(Bjorn[!Bjorn%in%Other], collapse = "<br>"))
  
  Eric = unique(venn_data$`Common Name`[venn_data$Observer=="Eric Larson"])
  Other = unique(venn_data$`Common Name`[!venn_data$Observer=="Eric Larson"])
...
output$totals <- renderText({
  ...
  individuals <- ceiling(sum(all_collaborative_birds$Count[all_collaborative_birds$year == year]))
...
output$leaderboard <- renderText({
  ...
  output <- paste("<h4>",year," species</h4><ul><li>Abby: <span style='color:#60AB9A; font-weight:bold;'>",
                  abby," species</span><br></li><li>Bjorn: <span style='color:#18476E; font-weight:bold;'>",
                  bjorn," species</span></li><li>Sue: <span style='color:#A355C7; font-weight:bold;'>",
                  sue," species</span></li><li>Eric: <span style='color:#F5B905; font-weight:bold;'>",
                  eric, " species</span></li></ul>"
                  )
})
...
output$map <- renderLeaflet({ 
  years <- if(input$map_years == "All"){2024:year(Sys.Date())} else {as.numeric(input$map_years)}
  ...
  map_years <- to_map %>%
    ...
    summarise(
      ...
      Marker_size = 4*(sum(n)^(1/3)))
  
  map_years$marker_col <- mapply(
  mix_from_topcols,
  map_years$Observers,
  map_years$Visits,
